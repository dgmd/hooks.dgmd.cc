...
const E="cbi",t="cc",_="cm",A="dbi",T="ub",R="ubi",a="um",l="create",O="db-id",m="delete",u="delete-id",b="page",I="result",N="result-type",Y="update",p="update-page-id",B="BLOCKS",C="checkbox",f="date",x="email",Q="id",H="multi_select",j="number",v="phone_number",V="relation",X="rich_text",Z="select",q="status",w="title",z="url",J="formula.string",W="formula.boolean",$="formula.date",ee="DATABASE_ID",te="END_DATE",Ae="METADATA",Te="PAGE_ID",ie="PRIMARY_DATABASE",ce="PROPERTIES",le="RELATION_DATABASES",de="RELATION_DATABASE_ID",re="RELATION_PAGE_ID",oe="START_DATE",me="TYPE",ue="VALUE",Pe="CURSOR_DATA",se="CURSOR_NEXT",be="PROTO_RESPONSE_KEY_SNAPSHOT_TIMESTAMP",Ne="i",Ye="c",Ue="s",Le="QUERY_RESPONSE_KEY_RESULT",Be="QUERY_RESPONSE_KEY_SUCCESS";
...
const SEARCH_TYPE_COMPLEX = 'SEARCH_TYPE_COMPLEX';
const SEARCH_TYPE_SIMPLE = 'SEARCH_TYPE_SIMPLE';
...
const IDGMD_FILTERED_DATA = 'IDGMD_FILTERED_DATA';
...
const isNotionDataFiltered = (jsonObject) => {
...
const getNotionDataDb = (jsonObject, dbId) => {
  if (isNotionDataValid(jsonObject)) {
    ...
    if (!isNil(job)) {
      const primary = job[ie];
      if (primary[ee] === dbId) {
        return primary;
      }
      for (var i=0; i<job[le].length; i++) {
        const db = job[le][i];
        if (db[ee] === dbId) {
          return db;
        }
...
const getNotionDataPage = (jsonObject, dbId, pageId) => {
  ...
  for (const xdbId of dbIds) {
    ...
    const pageIdx = dbBlocks.findIndex( block => {
      const blockIdMeta = block[Ae];
      const blockIdObj = blockIdMeta[Q];
      const blockId = blockIdObj[ue];
...
// SORT UTIL
//
const sortPages = (pgs, fields, directions) => {
  ...
  pgs.sort( (a, b) => {
    for (let i = 0; i < fieldsLen; i++) {
      try {
        ...
        const aVal = aField[ue];
        const bVal = bField[ue];
        ...
        const aType = aField[me];

        if (aType === f ||
            aType === $) {
          const aDateVal = moment.utc( aVal[oe] );
          const bDateVal = moment.utc( bVal[oe] );
          ...
          const aEndDateVal = moment.utc( aVal[te] );
          console.log( );
          const bEndDateVal = moment.utc( bVal[te] );
...
// SEARCH UTIL
//
const searchPages = ( jsonObj, dbId, searchObj ) => {
  ...
  const simpleSearchPage = ( 
    ...
    for (const pgKey of pgKeys) {
      ...
      const pgVal = pgProp[ue];
      if (!isNil(pgVal)) {
        const pgType = pgProp[me];
      ...
      if (!isNil(pgVal)) {
        ...
        if (pgType === V) {
          for (const relPgObj of pgVal) {
            const relPgId = relPgObj[re];
            if (!searchTracker.allSearched.includes( relPgId )) {
              const relDbId = relPgObj[ee];
  ...
  const complexSearchPage = ( pg, searchInfo, depth ) => {
    ...
    for (const si of searchInfo) {
      ...
      const pgVal = pgData[siField][ue];
      const pgType = pgData[siField][me];
    ...
    for (const si of siRels) {
      const siField = si[SEARCH_FIELD];
      const pgVal = pgProps[siField][ue];
      for (const relPgObj of pgVal) {
        const relPgId = relPgObj[re];
        const relDbId = relPgObj[de];
        const relPg = getNotionDataPage( jsonObj, relDbId, relPgId );
        const s = complexSearchPage( relPg, si[SEARCH_QUERY]);
        if (s) {
          pgClears.length = 0;
          break;
        }
  ...
  remove( pgs, pg => {
    ...
    if (complex) {
      return !complexSearchPage( pg, searchObj[SEARCH_INFO]);
    }
    return false;
  } );
};
...
const mergePageLists = (existingList, incomingList) => {
  ...
  const getId = obj => getPageMetadata(obj)[Q][ue];
    ...
    if (isNotionDataLive(notionData)) {
      ...
      setUrlUpdateObj( x => {
        return {
          ...
          [R]: pgId,
          [T]: structuredClone( pgUpdateProps ),
          [a]: structuredClone( pgUpdateMetas )
        };
      } );
    }
    else {
  ...
  useEffect( () => {
    async function fetchData() {
      ...
      try {
        ...
        const validStatus = parsedJsonObject[Be];
  ...
  useEffect( () => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (I in crudJson) {
          const result = crudJson[I];
          const resultType = crudJson[N];
          const success = result[resultType];
          if (success) {
            if (resultType === m) {
              const delId = result[u];
    ...
    async function fetchData() {
      try {
        ...
        const updateNotionData = x => {
          ...
          y[IDGMD_DATA][ie][B] = mergedPrimaryPgs;
          ...
          for (const previewDbId of previewDbIds) {
            ...
            y[IDGMD_DATA][le].find( 
              db => db[ee] === previewDbId )[B] = mergedPreviewPgs;
          }
...
const processQueryData = ( ojsonObject ) => {
  ...
  const parseRelationDbIds = (x) => {
    const job = x[IDGMD_DATA];
    const t = job[le].map( db => db[ee] );
    return t;
  };

  const jsonObject = structuredClone( ojsonObject ); 
  delete jsonObject[Be];
  jsonObject[IDGMD_FILTERED_DATA] = false;
  jsonObject[IDGMD_LIVE_DATA] = !isNil(jsonObject[be]) || true;
  delete jsonObject[be];
  jsonObject[IDGMD_DATA] = jsonObject[Le];
  delete jsonObject[Le];
...
export { getNotionDataPages, getNotionDataPrimaryDbId, hasNotionDataNextCursor, isNotionDataLive, isNotionDataLoaded, isNotionDataValid, useNotionData };
