...
const E="cbi",A="cc",t="cm",_="dbi",T="ub",R="ubi",S="um",r="create",O="db-id",D="delete",I="delete-id",N="page",s="result",b="result-type",n="update",p="update-blocks",Y="update-page-id",L="update-metas",B="BLOCKS",Q="id",V="relation",ee="DATABASE_ID",Ee="DATABASE_TITLE",Te="METADATA",Re="PAGE_ID",ce="PRIMARY_DATABASE",re="PROPERTIES",le="RELATION_DATABASES",Oe="RELATION_PAGE_ID",Pe="TYPE",me="VALUE",ue="CURSOR_DATA",Ne="CURSOR_NEXT",se="CURSOR_HAS_MORE",be="PROTO_RESPONSE_KEY_SNAPSHOT_TIMESTAMP",pe="i",Ye="c",ke="s",ge="QUERY_RESPONSE_KEY_RESULT",he="QUERY_RESPONSE_KEY_SUCCESS";
...
const getNotionDataPage = (jsonObject, dbId, pageId) => {
  ...
  for (const xdbId of dbIds) {
    ...
    const pageIdx = dbBlocks.findIndex( block => {
      const blockIdMeta = block[Te];
      const blockIdObj = blockIdMeta[Q];
      const blockId = blockIdObj[me];
...
const hasNotionDataNextCursor = (jsonObject) => {
  ...
  if (!(se in nextCursorData)) {
    return false;
  }
  if (!nextCursorData[se]) {
    return false;
  }
  if (isNil(nextCursorData[Ne])) {
    return false;
  }
  return true;
};
...
// Define date formats
const DATE_PRETTY_SHORT_DATE = 'MMM D, YYYY';
const DATE_PRETTY_SHORT_NUMERIC_DATE = 'M/D/YY';

// Function to format Notion dates using Moment.js
const prettyPrintNotionDate =
  (dateString, format = DATE_PRETTY_SHORT_NUMERIC_DATE) =>{
  try {
...
const getNotionDataPageId = page => {
  return getNotionDataPageMetadata(page)[Q][me];
};
...
const getNotionDataPagePropertyValue = (page, propertyKey) => {
  ...
  if (!(me in propertyObject)) {
    return null;
  }
  return propertyObject[me];
};
...
const mergePageLists = (existingList, incomingList) => {
  ...
  const getId = obj => getNotionDataPageMetadata(obj)[Q][me];
...
const useNotionData = url => {
  ...
  const loadNotionData = useCallback((dataUrl) => {
    ...
    return new Promise((resolve, reject) => {
      createXhrRequest('GET', dataUrl, {
        ...
        onLoad: (parsedData) => {
          const validStatus = parsedData[he];
  ...
  const handleCreate = useCallback((update, files = null) => {
    ...
    const pgUpdateMeta = update[Te];
    const pgUpdateMetas = isObject(pgUpdateMeta) ? pgUpdateMeta : {};
    const pgUpdateProp = update[re];
    ...
    if (isNotionDataLive(notionData)) {
      ...
      const createPromise = new Promise((resolve, reject) => {
        ...
        if (hasFiles) {
          ...
          jsonData = {
            [E]: dbId,
            [t]: structuredClone(pgUpdateMetas),
      ...
      const updatePromise = new Promise((resolve, reject) => {
        ...
        if (hasFiles) {
          ...
          jsonData = {
            ...
            [S]: structuredClone(pgUpdateMetas)
          };
        }
  ...
  const handleDelete = useCallback((dbId, pgId) => {
    ...
    if (isNotionDataLive(notionData)) {
      ...
      const deletePromise = new Promise((resolve, reject) => {
        ...
        deleteUrl.searchParams.append(_, pgId);
        ...
        createXhrRequest('DELETE', deleteUrl.href, {
          onLoad: (crudJson) => {
            if (s in crudJson) {
              ...
              if (success && resultType === D) {
                const delId = result[I];
  ...
  const handleNextCursor = useCallback(() => {
    ...
    const cursorPromise = new Promise((resolve, reject) => {
      ...
      createXhrRequest('GET', cursorUrl.href, {
        ...
        onLoad: (cursorJson) => {
          const validStatus = cursorJson[he];
          ...
          if (isNil(validStatus) || !validStatus) {
            ...
            setNotionData(x => {
              ...
              for (const previewDbId of previewDbIds) {
                ...
                z[IDGMD_DATA][le].find(
                  db => db[ee] === previewDbId
                )[B] = mergedPreviewPgs;
              }
...
const processQueryData = ( ojsonObject ) => {
  ...
  delete jsonObject[he];
  jsonObject[IDGMD_LIVE_DATA] = jsonObject[be] ? false : true;
  delete jsonObject[be];
  jsonObject[IDGMD_DATA] = jsonObject[ge];
  delete jsonObject[ge];
...
export { DATE_PRETTY_SHORT_DATE, DATE_PRETTY_SHORT_NUMERIC_DATE, getDbIdByName, getNotionDataDb, getNotionDataPage, getNotionDataPageId, getNotionDataPageMetadata, getNotionDataPageProperties, getNotionDataPagePropertyValue, getNotionDataPages, getNotionDataPrimaryDbId, getNotionDataRelationDbIds, hasNotionDataNextCursor, isNotionDataLive, isNotionDataLoaded, isNotionDataValid, prettyPrintNotionDate, useNotionData };
